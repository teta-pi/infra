...
| S-7 | `PATCH /businesses/{id}` keeps `agent_endpoint_verified=true` after endpoint change | 🟠 | 6.1 #6 | 1.5/1.x | **OPEN — confirmed 2026-08-05 (15.3 verification)**, code defect unchanged: `update_business` (`api/app/api/routes/businesses.py:233-247`) still blindly `setattr`s every PATCH field with no special-casing for `agent_endpoint`, so `agent_endpoint_verified` is never reset. **Practical exploitability is currently blocked by a separate, previously-undocumented bug**, also confirmed live this session: `POST /verify-endpoint` 500s on every call that includes `entity_id`, because `Business.id.cast("text")` (`endpoint_verification.py:86`) passes a Python string instead of a SQLAlchemy type — confirmed via live `curl` (500 with both a slug and a UUID `entity_id`) and via prod DB (`SELECT … WHERE agent_endpoint_verified=true` → 0 rows). So the flag can never legitimately become `true` today, meaning S-7's specific exploit path isn't reachable in practice right now — but the underlying defect is real and would immediately become live-exploitable the moment someone fixes the `/verify-endpoint` 500 without also fixing this. New 500 bug logged in `known-issues.md` |
